...
function handleTableScanned(tableNumber) {
  ...
  if (!tableNumber || isNaN(tableNumber)) {
    alert("⚠️ الرجاء مسح باركود صالح");
...
async function submitOrderToFirebase(items) {
  try {
    ...
    alert("⚠️ حدث خطأ أثناء إرسال الطلب. يرجى المحاولة مرة أخرى.");
  }
}
...
function submitOrder() {
  if (!currentTable) {
    alert("⚠️ الرجاء تحديد رقم الطاولة أولاً");
    return;
  }
  ...
  if (orderItems.length === 0) {
    alert("⚠️ الرجاء إضافة عناصر للطلب");
    return;
  }
...
function enterTableManually() {
  ...
  if (!tableNumber || isNaN(tableNumber)) {
    alert("⚠️ الرجاء إدخال رقم طاولة صحيح");
    return;
  }
